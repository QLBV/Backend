...
export class AttendanceService {
  ...
  static async checkIn(userId: number) {
    ...
    // Business Logic: Determine if LATE
    // For Doctors: check their shift today
    // For Staff: assume 8:00 start (or configurable)
    let isLate = false;
    let note = "";

    const doctor = await Doctor.findOne({ where: { userId } });
    if (doctor) {
      // Find current shift for doctor
      ...
      if (doctorShift && (doctorShift as any).shift) {
        const shiftStart = (doctorShift as any).shift.startTime; // e.g., "08:00"
        ...
        // Doctor with no shift today but checking in? Use default 08:00
      ...
      // Default staff shift starts at 08:00
      const defaultStart = new Date();
      defaultStart.setHours(8, 0, 0, 0);
      if (now > defaultStart) {
        isLate = true;
  ...
  static async checkOut(userId: number) {
    ...
    // Business Logic: Determine if EARLY_LEAVE
    const doctor = await Doctor.findOne({ where: { userId } });
    let isEarly = false;
    if (doctor) {
      const doctorShift = await DoctorShift.findOne({
      ...
      if (doctorShift && (doctorShift as any).shift) {
        const shiftEnd = (doctorShift as any).shift.endTime; // e.g., "17:00"
  ...
  /**
   * Auto mark absence for a specific date (usually yesterday)
   */
  static async autoMarkAbsence(dateStr?: string) {
    const targetDate = dateStr || new Date(Date.now() - 86400000).toISOString().split("T")[0];
    
    // 1. Get all employees who should work
    // In this simple system, doctors have shifts, staff might not.
    // Let's mark all active staff (Admin, Receptionist, Doctor) who don't have records.
    const eligibleUsers = await User.findAll({
      where: {
        isActive: true,
        roleId: {
          [Op.in]: [1, 2, 4] // ADMIN, RECEPTIONIST, DOCTOR
        }
      }
    });
    ...
    for (const user of eligibleUsers) {
      ...
      if (!existing) {
        // Only mark absent if it's a doctor with a shift OR a staff member
        let shouldHaveWorked = true;
        
        // If doctor, check if they had a shift
  ...
  /**
   * Manually mark absence for a user on a specific date
   */
